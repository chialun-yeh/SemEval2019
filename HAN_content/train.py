...
def get_args():
    ...
    parser.add_argument("--train_set", type=str, default="../data/articles-training-bypublisher.txt")
    parser.add_argument("--train_label", type=str, default = '../data/ground-truth-training-bypublisher.xml')
    parser.add_argument("--test_set", type=str, default="../data/articles-validation-bypublisher.txt")
    parser.add_argument("--test_label", type=str, default='../data/ground-truth-validation-bypublisher.xml')
    parser.add_argument("--test_interval", type=int, default=1, help="Number of epoches between testing phases")
    parser.add_argument("--word2vec_path", type=str, default="../data/glove.6B.200d.txt")
...
def train(opt):
    ...
    for epoch in range(opt.num_epoches):
        for iter, (index, feature, label) in enumerate(training_generator):
            if torch.cuda.is_available():
        ...
        if epoch % opt.test_interval == 0:
            ...
            output_file.write( "Epoch: {}/{} \nDev loss: {} Dev accuracy: {} \nDev confusion matrix: \n{}\n\n".format(
                epoch + 1, opt.num_epoches, te_loss,
                test_metrics["accuracy"], test_metrics["confusion_matrix"]))
            print("Epoch: {}/{}, Lr: {}, Loss: {}, Accuracy: {}".format(
                epoch + 1, opt.num_epoches,
                optimizer.param_groups[0]['lr'], te_loss, test_metrics["accuracy"]))
            writer.add_scalar('Dev/Loss', te_loss, epoch)
            writer.add_scalar('Dev/Accuracy', test_metrics["accuracy"], epoch)
            model.train()
